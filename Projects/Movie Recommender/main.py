...
def find_top_five(personalized_array):
	top_five = sorted(range(len(personalized_array)), key=lambda i: personalized_array[i])[-5:]	
	top_five.reverse()
	return top_five
...
def parse_similarity(num_movies):
	similarity = {}
	preprocessed_file = open('preprocessed_data.csv', "r")
...
def random_survey(movies_name, num_movies, number_of_questions):
	...
	while n != number_of_questions:
		...
		print "How Much You rate", movies_name[mid], "between 1 to 5 (insert -1 if you have not seen it)"
		try:
			num = input()
...
def main():
	...
	movies_name = None
	if path.isfile('preprocessed_data.csv') is False:
	...
	'''
	for keys,values in similarity.items():
		print keys
		print values
	'''
	personalized_array = calculate_rating(user_response, similarity, num_movies)
