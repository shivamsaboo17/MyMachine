...
def  save_similarity(similarity):
    new_file = open("preprocessed_data.csv", "w")
...
def fill_movies(movies):
    movie_file = open('MovieData/movies.csv', "r")
...
def preprocess():
    ...
    user_file = open('MovieData/rating.csv', "r")
    user_reader = csv.reader(user_file)
    movies = {}
    for row in user_reader:
        if movies.has_key(int(row[1])):
            movies[int(row[1])].add_user(int(row[0]), float(row[2]))
        else:
            movies[int(row[1])] = movie(movies_name[int(row[1])], [int(row[0])], [float(row[2])])
    user_file.close()
    '''
